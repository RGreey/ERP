...
public function listarMonitoriasActivas()
{
    ...
    public function seguimiento($monitoria_id, Request $request)
    {
        ...
        // Obtener el mes solicitado (por defecto el actual)
        $mesSolicitado = $request->get('mes', now()->month);
        $anioSolicitado = $request->get('anio', now()->year);
        ...
        // Validar que el mes solicitado esté en los meses configurados
        $mesValido = false;
        if ($monitor && $monitor->horas_mensuales) {
            ...
            $mesNombre = $mesesEnEspanol[$mesSolicitado] ?? '';
            $mesValido = isset($horasMensuales[$mesNombre]) && $horasMensuales[$mesNombre] > 0;
        }
        
        // Si el mes solicitado no es válido, redirigir al primer mes configurado
        if (!$mesValido && $monitor && $monitor->horas_mensuales) {
            $horasMensuales = is_string($monitor->horas_mensuales) ? json_decode($monitor->horas_mensuales, true) : $monitor->horas_mensuales;
            $primerMes = null;
            $primerAnio = null;
            
            foreach ($horasMensuales as $mesNombre => $horas) {
                if ($horas > 0) {
                    $mesNumero = array_search($mesNombre, $mesesEnEspanol);
                    if ($mesNumero) {
                        $primerMes = $mesNumero;
                        $primerAnio = $anioSolicitado; // Usar el año actual como base
                        break;
                    }
                }
            }
            ...
            if ($primerMes) {
                return redirect()->route('seguimiento.monitoria', [
        ...
        // Obtener todas las actividades del monitor
        $actividades = \App\Models\Seguimiento::where('monitor', $monitor->id)->get();
        
        $mesActual = $mesesEnEspanol[$mesSolicitado];
        $anioActual = $anioSolicitado;
        
        // Obtener meta de horas para el mes solicitado
        $metaHoras = 0;
        if ($monitor && $monitor->horas_mensuales) {
            $horasMensuales = is_string($monitor->horas_mensuales) ? json_decode($monitor->horas_mensuales, true) : $monitor->horas_mensuales;
        ...
        // Filtrar actividades del mes solicitado
        $actividadesMes = $actividades->filter(function($a) use ($mesSolicitado, $anioSolicitado) {
            $fechaActividad = \Carbon\Carbon::parse($a->fecha_monitoria);
            return $fechaActividad->month == $mesSolicitado && $fechaActividad->year == $anioSolicitado;
        });
        ...
        // Verificar si el seguimiento del mes específico está firmado
        $seguimientoFirmado = false;
        if ($actividadesMes->isNotEmpty()) {
            // Buscar si alguna actividad del mes tiene firma
            $actividadConFirma = $actividadesMes->first(function($actividad) {
                return !empty($actividad->firma_digital);
            });
            $seguimientoFirmado = $actividadConFirma !== null;
        }
        ...
        // Período de gracia especial para septiembre 2025 (configuración tardía)
        $diasGracia = 7; // Por defecto 7 días
        if ($mesSolicitado == 9 && $anioSolicitado == 2025) {
            $diasGracia = 21; // 3 semanas para septiembre 2025
        }
        
        $fechaFinGracia = $ultimoDiaDelMes->copy()->addDays($diasGracia);

        // Determinar si está en período de gracia (con posibles overrides por .env)
        $enPeriodoGracia = $hoy->lte($fechaFinGracia);
        $overrideGlobal = filter_var(env('SEGUIMIENTO_OVERRIDE_GLOBAL', false), FILTER_VALIDATE_BOOLEAN);
        // Override temporal: listar meses permitidos en formato YYYY-MM, separados por coma
        $overrideMeses = array_filter(explode(',', env('SEGUIMIENTO_OVERRIDE_MESES', '')));
        $ymActual = sprintf('%04d-%02d', $anioSolicitado, $mesSolicitado);
        if ($overrideGlobal || in_array($ymActual, $overrideMeses)) {
            $enPeriodoGracia = true;
        }
        $mesPasado = $hoy->gt($fechaFinGracia);
        
        // Determinar si se puede editar (solo si está en período de gracia Y es monitor)
        $puedeEditar = $enPeriodoGracia && $esMonitor;
        
        // --- GENERAR TABS PARA MESES CONFIGURADOS ---
        $tabsMeses = [];
        if ($monitor && $monitor->horas_mensuales) {
            $horasMensuales = is_string($monitor->horas_mensuales) ? json_decode($monitor->horas_mensuales, true) : $monitor->horas_mensuales;
            
            // Solo incluir meses que tienen horas configuradas (mayor a 0)
            $mesesConfigurados = [];
            foreach ($horasMensuales as $mesNombre => $horas) {
                if ($horas > 0) {
                    $mesNumero = array_search($mesNombre, $mesesEnEspanol);
                    if ($mesNumero) {
                        // Determinar el año basado en las fechas de vinculación
                        $anioMes = $anioSolicitado; // Usar el año solicitado como base
                        
                        // Si el mes es anterior al mes de vinculación, usar el año anterior
                        if ($monitor->fecha_vinculacion) {
                            $fechaVinculacion = \Carbon\Carbon::parse($monitor->fecha_vinculacion);
                            if ($mesNumero < $fechaVinculacion->month) {
                                $anioMes = $fechaVinculacion->year + 1;
                            } else {
                                $anioMes = $fechaVinculacion->year;
                            }
                        }
            ...
            // Ordenar por año y mes (más antiguo primero - izquierda a derecha)
            usort($mesesConfigurados, function($a, $b) {
                if ($a['anio'] == $b['anio']) {
                    return $a['mes'] - $b['mes']; // Mes menor primero (septiembre antes que octubre)
                }
                return $a['anio'] - $b['anio']; // Año menor primero
            });
            
            foreach ($mesesConfigurados as $mesData) {
                $ultimoDiaMes = \Carbon\Carbon::create($mesData['anio'], $mesData['mes'])->endOfMonth();
                
                // Período de gracia especial para septiembre 2025
                $diasGracia = 7; // Por defecto 7 días
                if ($mesData['mes'] == 9 && $mesData['anio'] == 2025) {
                    $diasGracia = 21; // 3 semanas para septiembre 2025
                }
                
                $finGracia = $ultimoDiaMes->copy()->addDays($diasGracia);
                // Aplicar override también en los tabs
                $ymTab = sprintf('%04d-%02d', $mesData['anio'], $mesData['mes']);
                $overrideMeses = $overrideMeses ?? array_filter(explode(',', env('SEGUIMIENTO_OVERRIDE_MESES', '')));
                $estaOverriden = $overrideGlobal || in_array($ymTab, $overrideMeses);
                ...
                $tabsMeses[] = [
                    ...
                    'horas' => $mesData['horas'],
                    'activo' => $mesData['mes'] == $mesSolicitado && $mesData['anio'] == $anioSolicitado,
                    'puedeEditar' => $estaOverriden ? true : $hoy->lte($finGracia),
                    'enGracia' => $estaOverriden ? true : ($hoy->lte($finGracia) && $hoy->gt($ultimoDiaMes)),
                    'url' => route('seguimiento.monitoria', [
                        'monitoria_id' => $monitoria_id,
                        'mes' => $mesData['mes'],
                        'anio' => $mesData['anio'],
                        'monitor_id' => $request->get('monitor_id')
        ...
        return view('monitoria.seguimiento', [
            ...
            'tabsMeses' => $tabsMeses,
            'seguimientoFirmado' => $seguimientoFirmado,
        ]);
    }

    public function generarPDFSeguimiento($monitor_id, $mes, $anio = null, Request $request)
    {
        try {
            ...
            $monitorData = [
                'nombre' => $usuario->name ? mb_strtoupper($usuario->name, 'UTF-8') : 'NOMBRE NO ENCONTRADO',
                'cedula' => $usuario->cedula ?? 'CEDULA NO ENCONTRADA',
                'plan_academico' => '', // Se quita según solicitud
            ];
            
            // Datos automáticos de la monitoría
            $solicitante = $encargado->name ? mb_strtoupper($encargado->name, 'UTF-8') : 'ENCARGADO NO ENCONTRADO';
            $proceso = $dependencia->nombrePD ? mb_strtoupper($dependencia->nombrePD, 'UTF-8') : 'DEPENDENCIA NO ENCONTRADA';
            
            
            // Obtener el periodo basado en las fechas de vinculación y culminación del monitor
            $periodo = 'Período No Definido'; // Valor por defecto
            if ($monitor->fecha_vinculacion && $monitor->fecha_culminacion) {
                $fechaInicio = \Carbon\Carbon::parse($monitor->fecha_vinculacion);
                $fechaFin = \Carbon\Carbon::parse($monitor->fecha_culminacion);
                ...
                $nombreMesInicio = $meses[(int)$mesInicio] ?? '';
                $nombreMesFin = $meses[(int)$mesFin] ?? '';
                
                if ($nombreMesInicio && $nombreMesFin) {
                    $periodo = $nombreMesInicio . ' - ' . $nombreMesFin;
                }
            }
            $sede = 'Caicedonia';
            
            // Si no se proporciona año, usar el año actual
            $anioFiltro = $anio ?: now()->year;
            ...
            // Si no hay firma en el request (acceso desde historial), buscar la firma guardada
            if (!$firmaDigitalBase64) {
                $firmaGuardada = \App\Models\Seguimiento::where('monitor', $monitor_id)
                    ->whereMonth('fecha_monitoria', $mes)
                    ->whereYear('fecha_monitoria', $anioFiltro)
                    ->whereNotNull('firma_digital')
                    ->where('firma_digital', '!=', '')
                    ->first();
                
                if ($firmaGuardada) {
                    $firmaDigitalBase64 = $firmaGuardada->firma_digital;
                    $firmaSize = 70; // Valor por defecto
                    $firmaPos = -15; // Valor por defecto
                }
            }
            ...
            // Agregar headers para mostrar PDF en iframe
            return $pdf->stream('seguimiento_monitoria.pdf', [
                'Content-Type' => 'application/pdf',
                'Content-Disposition' => 'inline; filename="seguimiento_monitoria.pdf"',
    ...
    public function guardarSeguimiento(Request $request)
    {
        try {
            ...
            if ($esEncargado) {
                // Si es encargado, actualizar las firmas SOLO de las actividades del mes específico
                $actividadesExistentes = \App\Models\Seguimiento::where('monitor', $monitorId)
                    ->whereMonth('fecha_monitoria', $mes)
                ...
                \App\Models\DocumentoMonitor::updateOrCreate(
                    ...
                    [
                        'parametros_generacion' => [
                            ...
                            'proceso' => '',
                            'firma_presente' => !empty($firmaDigital),
                            'firma_size' => $firmaSize,
                            'firma_pos' => $firmaPos
                        ],
                        'estado' => 'firmado',
                ...
                // Solo eliminar actividades del mes específico, no todas las actividades del monitor
                \App\Models\Seguimiento::where('monitor', $monitorId)
                    ->whereMonth('fecha_monitoria', $mes)
                    ->whereYear('fecha_monitoria', $anio)
                    ->delete();
                ...
                \App\Models\DocumentoMonitor::updateOrCreate(
                    ...
                    [
                        'parametros_generacion' => [
                            'nombre' => $usuario->name ? mb_strtoupper($usuario->name, 'UTF-8') : '',
                            'cedula' => $usuario->cedula ?? '',
                            'plan_academico' => '', // Se quita según solicitud
                            'solicitante' => $encargado->name ? mb_strtoupper($encargado->name, 'UTF-8') : 'NOMBRE DEL SOLICITANTE',
                            'proceso' => $dependencia->nombrePD ? mb_strtoupper($dependencia->nombrePD, 'UTF-8') : ''
                        ],
                        'estado' => 'generado',
                        'fecha_generacion' => now()
                    ]
